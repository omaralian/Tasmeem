...
```
Base tokens            -> Semantic tokens        -> Component tokens          -> Variant role tokens       -> Components
src/css/base/tokens.css   src/css/base/semantic.css  src/css/base/component-tokens.css   src/css/base/variants.css      src/css/components/*, etc.
```
...
- **Semantic tokens** (`semantic.css`): colors, spacing, radius, type, motion, shadows **derived** from base tokens with native `oklch()` + `calc()`. Colors are stored as OKLCH channels so variants come from `calc()` on lightness/chroma (e.g. hover = lightness − 0.06). Every foreground is computed by the **auto-contrast engine** (see below), not hardcoded; surface text auto-inverts because it keys off `--surface-l`.
  - **Auto-contrast engine.** Because OKLCH lightness is perceptual, the readable foreground for any background is a sharp `clamp()` switch on its L against `--contrast-threshold` — no per-hue bias. The reusable 2-line pattern per color is `--_on-<x>-l: clamp(0.15, calc((var(--contrast-threshold) - <bg L>) * 100), 0.98); --color-on-<x>: oklch(var(--_on-<x>-l) 0 0);`. It drives every `--color-on-*` (text on colored fills) **and** `--color-text`/`--color-muted` (text on neutral surfaces). Status fills get a real perceptual L via an offset from `--primary-l` (`--success-l`, `--warning-l`, …); that real L is what the engine reads, so each foreground is picked correctly with no per-color overrides — the brighter fills (success/warning/info) take dark text, the deeper ones (primary/danger) take light text. The base lightnesses are tuned so every `.btn` variant clears WCAG AA in default/hover/active states.
- **Component tokens** (`component-tokens.css`): per-component **layout** aliases (`--button-padding-*`, `--button-radius`, `--link-color`, `--focus-ring-color`, …). Component *color* now flows through the variant role tokens below, not through per-component color aliases.
- **Variant role tokens** (`variants.css`): the **framework-level variant/style system**. This is how meaning (variant) and appearance (style) stay independent and composable across *all* components.
  - A **variant** (`primary`, `success`, `warning`, `danger`, `info`, plus `neutral`) is just a class that re-points a generic set of role tokens — `--variant-bg`/`-hover`/`-active`, `--variant-fg`, `--variant-border`, `--variant-color` (accent), the `--variant-soft`/`-soft-strong`/`-soft-stronger` tint ramp (faint → strongest, for the ghost/subtle hover & active fills), `--variant-ring` (focus). It carries **no** component-specific CSS, so the same `.primary`/`.danger`/… classes recolor buttons, badges, alerts, tags, nav, and any future component uniformly. Add a variant by copying a block and pointing it at that color's semantic tokens — no component changes.
  - **Neutral is the default foundation.** The `:root` block seeds every `--variant-*` token with the neutral (surface-derived) palette, so a component with **no** variant class is fully styled out of the box; a variant class simply overrides those role tokens on the element. Usage is `class="btn primary"`, `class="btn danger outline"` — the bare `btn` is neutral.
  - **Variant vs. style.** A *style* (`solid` default, `outline`, `ghost`, `subtle`) is the only per-component variant code. Each component renders the role tokens once per style with **one additive rule per style** (not one rule per variant×style), so styles compose with every variant: `.btn.danger.outline`, `.btn.success.ghost`, `.btn.info.subtle`. New components follow the same pattern — read `--variant-*` for color, add one rule per style. Each style spans three interaction steps off the tint ramp (e.g. ghost: transparent → `soft` hover → `soft-strong` active; subtle: `soft` rest → `soft-strong` hover → `soft-stronger` active), so hover and active stay visually distinct.
  - **Why role tokens are read directly (not via a `:root` component alias).** A `var()` inside a custom property declared at `:root` is substituted with `:root`'s value and then *inherited already-resolved*; it will **not** re-resolve when a variant class sets `--variant-*` on the element. So components consume `--variant-*` directly (or alias inside their own rule block), and variant classes set the tokens on the element itself. Don't reintroduce a `:root`-level `--button-bg: var(--variant-bg)` indirection — it silently breaks per-element variants.
...
- **Add new color variants by deriving them**, not by hardcoding hex. Follow the `oklch(var(--primary-…) calc(…))` pattern in `semantic.css`, and derive any new foreground with the auto-contrast pattern above rather than guessing a text color.
...
- Several token names are kept as backward-compatible aliases (e.g. `--radius` → `--radius-md`, plus `--space-sm/md/lg`, `--color-primary`, `--container-max`). Preserve these names and their default values. Token-level rendering stays visually unchanged; the **one intentional exception** is that a bare `.btn` (no variant) is now the **neutral** palette rather than primary — adding `.primary` restores the primary button. Variant and style are expressed only as composable classes (`.btn primary outline`); the disabled state on non-form elements uses the composable `.disabled` class.
